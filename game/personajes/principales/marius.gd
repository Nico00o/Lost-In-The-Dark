...
func _process(delta):
	...
	if ray_suelo.is_colliding():
		...
		if collider is TileMap:
			...
			var layer = tilemap.get_layer_index("grass")
			...
			if tile_data:
				...
				if tipo == "pasto":
					# Guardamos la posición exacta del tile (convertida a global)
					ultimo_tile_pasto = tilemap.to_global(tilemap.map_to_local(cell))
